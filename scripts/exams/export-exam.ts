...
async function exportExamData() {
    ...
    const terminatedAssignments = assignments.filter((a) => a.assignment.isTerminated || a.assignment.malpracticeCount > 0);
    ...
    if (assignmentIds.length > 0) {
        ...
        const events = await db
            ...
            .innerJoin(examAssignments, eq(malpracticeEvents.assignmentId, examAssignments.id))
            .innerJoin(user, eq(examAssignments.userId, user.id))
    ...
    summarySheet.addRows([
        ...
        { property: "Start Time", value: selectedExam.startTime?.toLocaleString() || "N/A" },
        { property: "End Time", value: selectedExam.endTime?.toLocaleString() || "N/A" },
        ...
        { property: "Completed", value: assignments.filter((a) => a.assignment.status === "completed").length },
        { property: "In Progress", value: assignments.filter((a) => a.assignment.status === "in_progress").length },
        { property: "Not Started", value: assignments.filter((a) => a.assignment.status === "not_started").length },
        { property: "Terminated (Malpractice)", value: assignments.filter((a) => a.assignment.isTerminated).length },
        { property: "Total Malpractice Events", value: allMalpracticeEvents.length },
    ...
    console.log(`   - Completed: ${assignments.filter((a) => a.assignment.status === "completed").length}`);
    console.log(`   - Terminated: ${assignments.filter((a) => a.assignment.isTerminated).length}`);
